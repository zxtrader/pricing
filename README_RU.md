...
> Русская документация не поддерживается с 0.0.6 версии приложении.
...
# HTTP status code
- HTTP 200 код используется при успешном выполнение запроса.
- HTTP 404 код используется для оповещения клиента что такой страницы не существует.
- HTTP 429 код используется при нарушении ограничения количества запросов.
- HTTP 500 код используется для внутренних ошибок на стороне сервиса.
